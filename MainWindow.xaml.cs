...
namespace MapleRobots
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //if (intTemp >= 0)
                label_AttackCount.Content = "攻擊次數: " + dm.DM.ReadInt(WindowHwnd, AttackCountAdr, 0);
            //intTemp = Hack.ReadInt(HpAlarmBaseAdr, HpAlarmOffset, WindowTitle);
            //if (intTemp >= 0)
                label_HpAlert.Content = "HP警告: " + dm.DM.ReadInt(WindowHwnd, HpAlarmAdr, 0);
            //intTemp = Hack.ReadInt(MpAlarmBaseAdr, MpAlarmOffset, WindowTitle);
            //if (intTemp >= 0)
                label_MpAlert.Content = "MP警告: " + dm.DM.ReadInt(WindowHwnd, MpAlarmAdr, 0);
            //intTemp = Hack.ReadInt(HpValueBaseAdr, HpValueOffset, WindowTitle);
            //if (intTemp >= 0)
                label_HpValue.Content = "HP值: " + dm.DM.ReadInt(WindowHwnd, HpValueAdr, 0);
            //intTemp = Hack.ReadInt(MpValueBaseAdr, MpValueOffset, WindowTitle);
            //if (intTemp >= 0)
                label_MpValue.Content = "MP值: " + dm.DM.ReadInt(WindowHwnd, MpValueAdr, 0);
            //doubleTemp = Hack.ReadDouble(ExpPercentBaseAdr, ExpPercentOffset, WindowTitle);
            //if (doubleTemp > 0)
                label_Exp.Content = "經驗值: " + Math.Round(dm.DM.ReadDouble(WindowHwnd, ExpPercentAdr),2) + "%";
            //intTemp = Hack.ReadInt(RedCountBaseAdr, RedCountOffset, WindowTitle);
            //if (intTemp >= 0)
                label_PlayerCount.Content = "地圖玩家數量: " + dm.DM.ReadInt(WindowHwnd, PlayerCountAdr, 0);
            //intTemp = Hack.ReadInt(MobCountBaseAdr, MobCountOffset, WindowTitle);
            //if (intTemp >= 0)
                label_MobCount.Content = "地圖怪物數量: " + dm.DM.ReadInt(WindowHwnd, MobCountAdr, 0);
            //intTemp = Hack.ReadInt(CharacterStatusBaseAdr, CharacterStatusOffset, WindowTitle);
            //if (intTemp >= 0)
                label_CharacterStates.Content = "角色狀態: " + dm.DM.ReadInt(WindowHwnd, CharacterStatusAdr, 0);
            //intTemp = Hack.ReadInt(CharacterXBaseAdr, CharacterXOffset, WindowTitle);
            //if (intTemp >= 0)
                label_CharacterX.Content = "角色X座標: " + dm.DM.ReadInt(WindowHwnd, CharacterXAdr, 0);
            //intTemp = Hack.ReadInt(CharacterYBaseAdr, CharacterYOffset, WindowTitle);
            //if (intTemp >= 0)
            label_CharacterY.Content = "角色Y座標: " + dm.DM.ReadInt(WindowHwnd, CharacterYAdr, 0);
            //intTemp = Hack.ReadInt(MapIDBaseAdr, MapIDOffset, WindowTitle);
            //if (intTemp >= 0)
                label_MapID.Content = "地圖編號: " + dm.DM.ReadInt(WindowHwnd, MapIDAdr, 0);
        ...
        private void BottingGoby()
        {
            ...
            while (true)
            {
                ...
                //for (int i = 0; i < 5 && dm.DM.ReadInt(WindowHwnd, MobCountAdr, 0) >= 15; i++)
                Attack(1);
                do
                {
                    Attack(1);
